...
import { Search, MapPin, Users, Home, Bed, Calendar, Star, CheckCircle, MessageCircle, SlidersHorizontal, ExternalLink, UtensilsCrossed, Scissors, ShoppingBag, ChevronDown, X, Send, Sparkles } from 'lucide-react';
...
const universities = ['All Universities', 'University of Lagos', 'University of Ibadan', 'Covenant University', 'Lagos State University'];
...
const TugoExplorePage: React.FC = () => {
  ...
  const ListingCard: React.FC<{ listing: Listing; isSponsored?: boolean }> = ({ listing, isSponsored = false }) => (
    <div className={`rounded-3xl overflow-hidden transition-all duration-300 backdrop-blur-sm group ${
      isSponsored 
  ...
  const BusinessAdCard: React.FC<{ ad: BusinessAd; isSponsored?: boolean; isCompact?: boolean }> = ({ ad, isSponsored = false, isCompact = false }) => (
    <div className={`rounded-3xl overflow-hidden transition-all duration-300 backdrop-blur-sm group ${
    ...
    <div className="min-h-screen bg-black text-white overflow-hidden font-['Plus_Jakarta_Sans',sans-serif]">
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <button 
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-bold transition-all ${
                showFilters ? 'bg-white text-black' : 'bg-white/10 text-white border border-white/20 hover:bg-white/20'
              }`}
            >
          ...
          {showFilters && (
            <div className="mt-4 space-y-4">
              <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
                <button onClick={() => setSelectedType('all')} className={`px-4 py-2.5 rounded-full text-sm font-bold whitespace-nowrap transition-all ${selectedType === 'all' ? 'bg-white text-black' : 'bg-white/10 text-white border border-white/20 hover:bg-white/20'}`}>All</button>
                <button onClick={() => setSelectedType('roommate')} className={`px-4 py-2.5 rounded-full text-sm font-bold whitespace-nowrap transition-all flex items-center ${selectedType === 'roommate' ? 'bg-white text-black' : 'bg-white/10 text-white border border-white/20'}`}><Users className="w-4 h-4 mr-1.5" />Roommates</button>
                <button onClick={() => setSelectedType('bunkmate')} className={`px-4 py-2.5 rounded-full text-sm font-bold whitespace-nowrap transition-all flex items-center ${selectedType === 'bunkmate' ? 'bg-white text-black' : 'bg-white/10 text-white border border-white/20'}`}><Bed className="w-4 h-4 mr-1.5" />Bunkmates</button>
                <button onClick={() => setSelectedType('rental')} className={`px-4 py-2.5 rounded-full text-sm font-bold whitespace-nowrap transition-all flex items-center ${selectedType === 'rental' ? 'bg-white text-black' : 'bg-white/10 text-white border border-white/20'}`}><Home className="w-4 h-4 mr-1.5" />Rentals</button>
              </div>
              <div className="relative">
                <select value={selectedUniversity} onChange={(e) => setSelectedUniversity(e.target.value)} className="w-full p-3 bg-white/10 border border-white/20 rounded-xl text-white focus:outline-none appearance-none pr-10">
                  {universities.map(uni => <option key={uni} value={uni} className="bg-black">{uni}</option>)}
                </select>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {regularItems.map((item, idx) => (
              item.type === 'listing' 
                ...
                : <BusinessAdCard key={item.id} ad={item} isCompact={idx % 3 !== 0} />
            ))}
          </div>
        </div>
      </div>
      ...
      {showAIChat && (
        <div className="fixed inset-0 z-50 flex items-end justify-center md:items-center md:p-4">
          <div className="absolute inset-0 bg-black/80 backdrop-blur-sm" onClick={() => setShowAIChat(false)}></div>
          <div className="relative w-full max-w-2xl h-[80vh] md:h-[600px] bg-gradient-to-b from-gray-900 to-black rounded-t-3xl md:rounded-3xl border border-white/20 flex flex-col">
            <div className="flex items-center justify-between p-6 border-b border-white/10">
              ...
              <button onClick={() => setShowAIChat(false)} className="p-2 hover:bg-white/10 rounded-full transition">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="p-4 border-t border-white/10">
              <div className="flex gap-3">
                ...
                />
                <button onClick={handleSendMessage} className="w-12 h-12 bg-white text-black rounded-full flex items-center justify-center hover:scale-110 transition-transform">
